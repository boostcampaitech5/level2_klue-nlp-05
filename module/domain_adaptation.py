...
def NER(model_name):
    ...
    train, dev = train_test_split(train, test_size=CFG['RATIO'], random_state=14)
    
    # save data
    with open('/opt/ml/dataset/train/ner_train.txt', 'w', encoding='UTF8') as f:
        f.writelines(train)
    with open('/opt/ml/dataset/train/ner_dev.txt', 'w', encoding='UTF8') as f:
        f.writelines(dev)
