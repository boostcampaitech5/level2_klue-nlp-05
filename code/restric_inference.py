...
import yaml
from restrict_load_data import *
from custom.RestricModel import *
...
def load_test_dataset(dataset_dir, tokenizer, model_type, discrip):
  ...
  if model_type == 'base':
    ...
    test_label = list(map(int,test_dataset['label'].values))
    # tokenizing dataset
    tokenized_test, restrict_num_test = punct_tokenized_dataset(test_dataset, tokenizer)
    return test_dataset['id'], tokenized_test, test_label, restrict_num_test
...
def main(CFG):
  ...
  if CFG['MODEL_TYPE'] == 'base':
    ...
    test_id, test_dataset, test_label, restrict_num_test = load_test_dataset(test_dataset_dir, tokenizer, CFG['MODEL_TYPE'], CFG['DISCRIP'])
    Re_test_dataset = RE_Restrict_Dataset(test_dataset ,test_label, restrict_num_test)
