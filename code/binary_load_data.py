...
def preprocessing_dataset(dataset, binary):
  ...
  if binary:
    dataset.loc[dataset['label'] != 'no_relation', 'label'] = 1
    dataset.loc[dataset['label'] == 'no_relation', 'label'] = 0
...
def special_preprocessing_dataset(dataset, binary):
  sentences = []
  subject_type = []
  object_type = []
...
def punct_preprocessing_dataset(dataset, binary):
  sentences = []
...
def load_data(dataset_dir, model_type, binary):
  """ csv 파일을 경로에 맡게 불러 옵니다. """
  pd_dataset = pd.read_csv(dataset_dir)

  if model_type == 'entity_special':
    dataset = special_preprocessing_dataset(pd_dataset, binary)
  elif model_type == 'entity_punct':
    dataset = punct_preprocessing_dataset(pd_dataset, binary)
  else:
    dataset = preprocessing_dataset(pd_dataset, binary)
