...
def train():
    ...
    if CFG['MODEL_TYPE'] == 'base':
        ...
        tokenized_train, entity_type_train = special_tokenized_dataset(
            train_dataset, tokenizer)
        tokenized_dev, entity_type_dev = special_tokenized_dataset(
            dev_dataset, tokenizer)

        RE_train_dataset = RE_special_Dataset(
            tokenized_train, train_label, entity_type_train)
        RE_dev_dataset = RE_special_Dataset(
            tokenized_dev, dev_label, entity_type_dev)

        model = SepecialEntityBERT(
            MODEL_NAME, config=model_config, tokenizer=tokenizer)
